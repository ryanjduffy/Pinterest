...
enyo.kind({
	...
	activate:function() {
		if(this.parent instanceof extras.Accordion) {
			this.parent.sectionActivated(this);
		}
...
enyo.kind({
	...
	rendered:function() {
		...
		var n;
		if(n = this.hasNode()) {
			var headerHeights = 0, defaultSection = null;
			this.eachSection(function(c) {
	...
	resizeHandler:function() {
		...
		this.adjustedHeight = n.offsetHeight - this.headerHeights;
		this.activeSection && this.activeSection.setHeight(this.adjustedHeight);
	}
});
